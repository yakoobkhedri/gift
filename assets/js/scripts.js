...
let requestProduct = Array.from(document.getElementsByClassName('requestProduct'));
...
<p> </p>
`,
        fields: JSON.parse(`[{"statics_key":"email","use":1,"required":1,"ordering":1,"title":"ایمیل","ltr":1,"icon":"fa fa-envelope-o","class":"en-numbers","validation":"required|valid_email","value":"","name":"email","col":"col-12"}]`)
    }
);
categories.push(
    {
...
document.addEventListener('click', (event) => {
    if (!event.target.closest('#dropdownList') && !event.target.closest('#giftBtn')) {
...
document.addEventListener('click', (event) => {
    if (!event.target.closest('#cartdropdownList') && !event.target.closest('#cartBtn')) {
...
price.forEach((item) => {
    item.addEventListener('click', function () {
        price.forEach((items) => { items.classList.remove('active') });
...
bank.forEach((item) => {
    item.addEventListener('click', function () {
        bank.forEach((items) => { items.classList.remove('active') });
        item.classList.add('active');
    })
})